...
contract VDFVerifier is IVDFVerifier {
    // Preset 2048 bit mod
    ...
    function verify_vdf_proof(bytes32 input_random, bytes memory y, bytes memory pi, uint256 iterations, uint256 prime) external view override {
    ...
    function check_hash_to_prime(bytes32 input_random, bytes memory y, uint256 prime) internal view {
    ...
    function bignum_expmod(bytes memory base, uint256 e, bytes memory m) internal view returns (bytes memory o) {
        assembly {
            // Get free memory pointer
            let p := mload(0x40)
    ...
    function miller_rabin_test(uint256 n) internal view returns (bool) {
        require(n > 3);
        require(n & 0x1 == 1);
        uint256 d = n - 1;
        uint256 r = 0;
